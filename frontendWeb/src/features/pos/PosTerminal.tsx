...
export function PosTerminal({ title, subtitle, zoneMode = false }: { title: string; subtitle: string; zoneMode?: boolean }) {
  ...
  const products = useQuery({
    queryKey: ["pos-products", search, zoneMode ? zoneId : "comptoir"],
    queryFn: () => getList<PosProduct>("/pos/products", { search: search || undefined, per_page: 200, zone_id: zoneId === "" ? undefined : zoneId }),
    enabled: !zoneMode || zoneId !== "",
  });
  ...
  const checkout = () =>
    pay
      .mutateAsync({ items: cart.map((l) => ({ product_id: l.product.id, quantity: l.quantity })), tax_ids: taxIds, payment_method: method, customer_name: customer || null, amount_received: received === "" ? null : Number(received), zone_id: zoneId === "" ? null : zoneId })
      .then((s) => { setCart([]); setReceived(""); setCustomer(""); setTaxIds([]); navigate(`/caisse/ventes/${s.id}`); })
      .catch(() => {});

  const noZone = zoneMode && zoneId === "";

  return (
    <>
      <ModuleTabs tabs={CAISSE_TABS} />
      <PageHeader title={title} subtitle={subtitle} breadcrumb="Caisse" />
      <div className="grid gap-6 xl:grid-cols-[1fr_400px]">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
              ...
              {groups.length === 0 && (
                <p className="col-span-full rounded-2xl border border-dashed border-ge7-black/15 p-8 text-center text-sm text-ge7-black/50">
                  {zoneMode ? "Aucun produit avec un barème pour cette ville." : "Aucun produit."}
                </p>
              )}
            </div>
          )}
        ...
        <Card className="xl:sticky xl:top-20 xl:self-start">
          ...
          <div className="mt-4 space-y-3 border-t border-ge7-black/5 pt-4">
            ...
            <Button size="lg" className="w-full" disabled={!cart.length || noZone || (method === "ESPECES" && (received === "" || (change !== null && change < 0)))} loading={pay.isPending} onClick={checkout}>Encaisser {money(total)}</Button>
          </div>
        </Card>
      </div>
